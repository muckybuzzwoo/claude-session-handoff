...
foreach($s in 'Status','What this is about','Decisions','Key files','Running state','Verification','Open work','Pick up here'){
    Check "_01 has section: $s" ($h01.Contains($s))
}
Check '_01 carries the Format header field' ($h01 -match '(?m)^\*\*Format:\*\*\s*2')
Check '_01 puts Pick up here before the retrospective' (
    $h01.IndexOf('Pick up here') -lt $h01.IndexOf('What this is about'))
